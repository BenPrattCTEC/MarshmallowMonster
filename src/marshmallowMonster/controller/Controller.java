...
public class Controller
{
	...
	public void start()
	{
		...
		while (true)
		{
			awnser = gui.displayQuestion("What would you like to do?\n1) Make Monster\n2) Remove Monster\n3) Print Monsters");
			if (awnser.equals("1"))
			{
				monsters.add(new Monster());
				interactiveMonsterCreation(monsters.get(monsters.size() - 1));
			} else if (awnser.equals("2"))
			{
				awnser = gui.displayQuestion(printMonsterArrayListToString(monsters));
				monsters.remove(Integer.parseInt(awnser));
			} else
			{
				gui.displayText(printMonsterArrayListToString(monsters));
			}
		}
		// boolean correctInput = false;
		// String tempString;
		// while (!correctInput) {
		// tempString = gui.displayQuestion(monsters.get(1).toString() + "\nHow many eyes would you like to
		// eat?");
	...
	private String printMonsterArrayListToString(List<Monster> monsters)
	{
		String tempString = "";
	...
	private void interactiveMonsterCreation(Monster monster)
	{
		...
		while (!isComplete)
		{
			...
			try
			{
				...
				eyeCount = Integer.parseInt(gui.displayQuestion("ID: " + monster.getID() + "\n(Integer) Enter number of eyes"));
				tenticleAmount = Double.parseDouble(gui.displayQuestion("ID: " + monster.getID() + "\n(Double) Enter number of Tenticles"));
				armCount = Integer.parseInt(gui.displayQuestion("ID: " + monster.getID() + "\n(Integer) Enter number of Arms"));
				hasBloop = Boolean.parseBoolean(gui.displayQuestion("ID: " + monster.getID() + "\n(Boolean) Does it hava a Bloop?"));
